...
class ObservationData:
    ...
    def to_flat_array(self) -> np.ndarray:
        ...
        # Agent info (5 values: pos_x, pos_y, can_shoot, vel_x, vel_y)
        ...
        features.extend(self.agent.velocity)
        features.append(self.agent.dist_to_ball)
        
        # Teammates (max 3: 5 values each, pad với 0 nếu thiếu)
        max_teammates = 3
        for i in range(max_teammates):
            if i < len(self.teammates):
                ...
                features.extend(tm.velocity)
                features.append(tm.dist_to_ball)
            else:
                features.extend([0.0] * 7)  # 2 + 1 + 2 + 2
        
        # Opponents (max 3: 5 values each, pad với 0)
        max_opponents = 3
        for i in range(max_opponents):
            if i < len(self.opponents):
                ...
                features.extend(opp.velocity)
                features.append(opp.dist_to_ball)
            else:
                features.extend([0.0] * 7)
        
        # Goal info (6 vectors, 2 values each + 2 positions)
        features.append(self.goal_info.pos_lower)
        features.append(self.goal_info.pos_upper)
        features.extend(self.goal_info.vec_lower_left_to_ball)
        features.extend(self.goal_info.vec_upper_left_to_ball)
        features.extend(self.goal_info.vec_lower_right_to_ball)
        features.extend(self.goal_info.vec_upper_right_to_ball)
        ...
        # Ball info
        features.extend(self.ball.pos)
        features.extend(self.ball.velocity)
        features.append(float(self.ball.bounces_left))
...
class ObservationProcessor:
    ...
    def create_observation_data(
        ...
        agent_pos_norm = np.array([
            agent_pos[0] / half_width,
            agent_pos[1] / half_height
        ])
        
        # Agent info
        agent_info = PlayerInfo(
            pos_normalized=agent_pos_norm,
            pos_relative_ball=agent_pos - ball_pos,
        ...
        for tm_pos, tm_vel, tm_shoot in zip(teammates_pos, teammates_vel, teammates_can_shoot):
            tm_pos_norm = np.array([
                tm_pos[0] / half_width,
                tm_pos[1] / half_height
            ])
            teammates.append(PlayerInfo(
                pos_normalized=tm_pos_norm,
                pos_relative_ball=tm_pos - ball_pos,
                dist_to_ball=np.linalg.norm(tm_pos - ball_pos),
                can_shoot=tm_shoot,
                velocity=tm_vel
            ))
        ...
        for opp_pos, opp_vel, opp_shoot in zip(opponents_pos, opponents_vel, opponents_can_shoot):
            opp_pos_norm = np.array([
                opp_pos[0] / half_width,
                opp_pos[1] / half_height
            ])
            opponents.append(PlayerInfo(
                pos_normalized=opp_pos_norm,
                pos_relative_ball=opp_pos - ball_pos,
                dist_to_ball=np.linalg.norm(opp_pos - ball_pos),
                can_shoot=opp_shoot,
                velocity=opp_vel
            ))
        
        # Goal vectors (từ 4 góc goal tới bóng)
        goal_left_x = -half_width
        goal_right_x = half_width
        
        vec_lower_left = ball_pos - np.array([goal_left_x, goal_pos_lower])
        vec_upper_left = ball_pos - np.array([goal_left_x, goal_pos_upper])
        vec_lower_right = ball_pos - np.array([goal_right_x, goal_pos_lower])
        vec_upper_right = ball_pos - np.array([goal_right_x, goal_pos_upper])
        
        goal_info = GoalInfo(
            pos_lower=goal_pos_lower,
            pos_upper=goal_pos_upper,
    ...
    def flat_obs_dim(self) -> int:
        ...
        # Agent: 5 (pos_x, pos_y, can_shoot, vel_x, vel_y) + 1 (dist)
        agent_dim = 6
        
        # Teammates: 3 x 7 (pos_x, pos_y, can_shoot, vel_x, vel_y, dist_to_ball, can_shoot)
        teammates_dim = 3 * 7
        
        # Opponents: 3 x 7
        opponents_dim = 3 * 7
        
        # Goal: 2 (positions) + 8 (4 vectors x 2 dims)
        goal_dim = 2 + 8
